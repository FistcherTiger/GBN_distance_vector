...
/* Timers */
struct itimerval timer;                // 500ms ACK time out
void timer_handler();
bool is_timeout;                       // Flag for timeout (ACK)
int is_ack;                            // Flag for ack, Update from 0 whenever the sending window has moved, value indicates how long it moves

struct itimerval timer2;               // 5 Seconds time out
...
float str_to_prob(char *str);                          // Convert string to probability in probabilistic mode
...
/* Global variables*/
int debug_mode = 0;
int window_size = 5;                                    // Window size used in GBN protocol
float p_probability = -1.0;                            // Chane of faliure in probabilistic mode
